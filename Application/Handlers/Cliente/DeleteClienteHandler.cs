...
namespace Application.Handlers.Cliente
{
    public class DeleteClienteHandler : IRequestHandler<DeleteClienteCommand, bool>
    {
        ...
        public DeleteClienteHandler(IClienteUseCase useCase,
        ...
        public async Task<bool> Handle(DeleteClienteCommand command, CancellationToken cancellationToken)
        {
            if (command.IsValid())
            {
                var output = await _useCase.DeleteClienteAsync(command.Input).ConfigureAwait(false);
                return output;
            }
